      ...
      autoClose={true}>
          <TouchableOpacity onPress={onPressItem}>
              <View style = {{backgroundColor: itemIndex % 2 == 0 ? 'powderblue' : 'skyblue'}}>
                  <Text style={{ fontWeight:'bold', fontSize:18, margin:10}}>{id}</Text>
...
class DanhSachDiemDanhComponent extends Component {
    ...
    laynhungsinhviendavangmat = ()=>{
        
    }
    hienthiDanhSachSVDiemDanh = () => {
        this.state.dsSinhVien.map(item => {
            const newSinhVien = {
            ...
            insertDiemDanh(newSinhVien).then().catch((error) => {
...
export default DanhSachDiemDanhComponent;
